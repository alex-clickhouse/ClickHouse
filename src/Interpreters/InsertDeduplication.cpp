...
DeduplicationInfo::FilterResult DeduplicationInfo::filterImpl(const std::set<size_t> & collision_offsets) const
{
    ...
    LOG_DEBUG(
        logger,
        "Filtered {}/{} rows with {}/{} tokens due to duplicate, remaining rows/count={}/{}, debug: {}, new tokens debug: {}",
        removed_rows,
        block.rows(),
...
void DeduplicationInfo::truncateTokensForRetry()
{
    for (auto & token : tokens)
    {
        ...
        chassert(next->type == TokenDefinition::Extra::Type::SOURCE_NYMBER || next->type == TokenDefinition::Extra::Type::VIEW_NUMBER);
        ++next;
...
Block DeduplicationInfo::goRetry(SharedHeader && header, Chunk && filtered_data, Ptr filtered_info, ContextPtr context) const
{
    ...
    auto filter =[this, filtered_info] (const Chunk & chunk) -> bool
    {
        ...
        if (!chunk)
        {
            LOG_DEBUG(this->logger, "skip empty chunk with deduplication info: {}", info ? info->debug() : "null");
            return false;
        }

        if (!info)
        {
            LOG_DEBUG(this->logger, "skip chunk with missing deduplication info");
            return false;
        }

        if (info->tokens != filtered_info->tokens)
        {
            LOG_DEBUG(this->logger, "skip chunk with deduplication info: {}", info->debug());
            return false;
        }

        LOG_DEBUG(this->logger, "found chunk with deduplication info: {}", info->debug());
        return true;
    };
